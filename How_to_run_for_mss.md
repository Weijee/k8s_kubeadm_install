...
主机名 | IP | 用途 
--------------------|-----------------|------------
ansible-controller  | 192.168.145.130 | Ansible控制节点
k8s-master          | 192.168.145.131 | 集群主节点
k8s-worker1         | 192.168.145.132 | 工作节点1
k8s-worker2         | 192.168.145.133 | 工作节点2
k8s-worker3         | 192.168.145.134 | 工作节点3

### 安装主机
依照上面的规划安装主机，安装过程中手动设置ip(可根据实际情况的网关和路由进行调整)
![手动设置IP地址_1](https://github.com/user-attachments/assets/0930a006-835e-4ad2-8540-78abd0b28c8d)
![手动设置IP地址_2](https://github.com/user-attachments/assets/841f4c8d-17b5-4f71-b786-efac6c197304)
![手动设置IP地址_3](https://github.com/user-attachments/assets/6be4a778-f30f-4c82-a571-4bced344a071)


### 检查网络环境
所有节点都检查一遍，确保网络没有问题，涉及到后面拉取镜像
```bash
ansible@ansible-controller:~$ ping pkgs.k8s.io
PING redirect.k8s.io (34.107.204.206) 56(84) bytes of data.
64 bytes from 206.204.107.34.bc.googleusercontent.com (34.107.204.206): icmp_seq=1 ttl=128 time=225 ms

ansible@ansible-controller:~$ ping registry.aliyuncs.com
PING registry.aliyuncs.com (120.55.105.209) 56(84) bytes of data.
64 bytes from 120.55.105.209 (120.55.105.209): icmp_seq=1 ttl=128 time=40.1 ms
```

### 手动修改Ansible-Controller节点的/etc/hosts(请注意: 主机名，ansible节点，/etc/hosts要保持一致)
```bash 
192.168.145.130 ansible-controller
192.168.145.131 k8s-master
192.168.145.132 k8s-worker1
192.168.145.133 k8s-worker2
192.168.145.134 k8s-worker3
```

### 配置免密(Ansible-Controller节点)
生成密钥
```bash
ansible@ansible-controller:~$ sudo su
root@ansible-controller:/home/ansible# ssh-keygen
...
SHA256:Iaxb8rWNOZ0r83Gd4JCdFBKWKa0BUaeqBjuE/ylCD8s root@ansible-controller
...
将k8s主机暂时开启root密码登录 (四个节点都要操作)
```bash
root@k8s-master:/home/master# echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
root@k8s-master:/home/master# systemctl restart ssh
root@k8s-master:/home/master# passwd root
New password:
Retype new password:
passwd: password updated successfully

root@k8s-worker1:/home/worker# echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
root@k8s-worker1:/home/worker# systemctl restart ssh
root@k8s-worker1:/home/worker# passwd root
New password:
Retype new password:
passwd: password updated successfully

root@k8s-worker2:/home/worker# echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
root@k8s-worker2:/home/worker# systemctl restart ssh
root@k8s-worker2:/home/worker# passwd root
New password:
Retype new password:
passwd: password updated successfully

root@k8s-worker3:/home/worker# echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
root@k8s-worker3:/home/worker# systemctl restart ssh
root@k8s-worker3:/home/worker# passwd root
New password:
Retype new password:
passwd: password updated successfully
```

将Ansible密钥复制到各个K8s节点 
循环四次依次输入yes和密码
```bash
root@ansible-controller:/home/ansible# for i in {k8s-master,k8s-worker1,k8s-worker2,k8s-worker3}; do  ssh-copy-id root@$i; done
/usr/bin/ssh-copy-id: INFO: Source of key(s) to be installed: "/root/.ssh/id_rsa.pub"
The authenticity of host 'k8s-master (192.168.145.131)' can't be established.
ECDSA key fingerprint is SHA256:s3yW/ozyjbL7dsXNmSKgbZADqgR404RxTme5bs/S/Zw.
...
Now try logging into the machine, with:   "ssh 'root@k8s-master'"
and check to make sure that only the key(s) you wanted were added.
...其他三次省略
```
确认可以使用以下方式远程到四台K8s机器
```bash
root@master1:~# ssh root@k8s-master
root@master1:~# ssh root@k8s-worker1
root@master1:~# ssh root@k8s-worker2
root@master1:~# ssh root@k8s-worker3
```

再关闭root密码登录(所有K8s节点都要操作)
```bash
root@k8s-master:~# sed -i '$d' /etc/ssh/sshd_config
root@k8s-master:~# systemctl restart ssh

root@k8s-worker1:~# sed -i '$d' /etc/ssh/sshd_config
root@k8s-worker1:~# systemctl restart ssh

root@k8s-worker2:~# sed -i '$d' /etc/ssh/sshd_config
root@k8s-worker2:~# systemctl restart ssh

root@k8s-worker3:~# sed -i '$d' /etc/ssh/sshd_config
root@k8s-worker3:~# systemctl restart ssh
```


### 更换Ansible-Controller系统源
https://help.mirrors.cernet.edu.cn/ubuntu/   
选择系统版本20.04  
```bash
root@ansible-controller:/etc/apt# cat <<'EOF' > /etc/apt/sources.list
...
执行更新
```bash
root@ansible-controller:~# apt update
```

### 在Ansible-Controller安装ansible
```bash
root@ansible-controller:~# apt install ansible -y
root@ansible-controller:~# mkdir -p /etc/ansible/
```
